import { FaStar,FaSpinner,FaChevronRight, FaChartPie,FaPlus, FaEllipsisV,FaChevronDown, FaEyeSlash,FaEye,  FaCookieBite, FaHome, FaCheck, FaPencilAlt, FaUsers, FaUndo, FaTable, FaCross, FaTimes,  } from "react-icons/fa"
...
return<>
{authState[0]?.emailVerified ? <main className={`min-h-screen mt-24  w-full ${isSidebarActive ? 'hidden' : 'block'}`}>
...
{icons.showBalance ? <button className="hover:bg-sky-800 w-16 h-16 rounded-full"><FaEye onClick={showBalance} className="inline-block  text-3xl cursor-pointer "/> </button>: <button> <FaEyeSlash onClick={showBalance} className="inline-block  text-3xl cursor-pointer " /></button>}
{!icons.showChart ? <button className="hover:bg-sky-800 w-16 h-16 rounded-full"><FaChartPie onClick={showChart} className="  inline-block  text-3xl cursor-pointer " /> </button>: <button  className="hover:bg-sky-800 w-16 h-16 rounded-full"><FaCookieBite onClick={showChart} className="inline-block  text-3xl cursor-pointer " /></button>}
<button className="hover:bg-sky-800 w-16 h-16 rounded-full">
<FaTable className="inline-block  text-3xl   " />
</button>
<button className="hover:bg-sky-800 w-16 h-16 rounded-full" ref={iconRef}>
...
{!icons.showTab ? <ul ref={tabRef} className={` ${theme ? 'bg-black shadow-neutral-900' : 'bg-sky-700'} md:w-[90%] w-full h-screen md:right-[40%]  top-0 md:top-auto left-0 md:left-auto  md:h-max rounded shadow-lg shadow-black absolute p-4`}>
 ...
 :null}
<button onClick={addCoin} className="bg-green-400 mt-4 ml-5 md:mt-auto text-white rounded cursor-pointer p-3 inline"> Add New Coin</button>
        </div>
